...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'spider-brim' | 'brimming' | 'y.a.n.k.e.e':
				{
				    var bg:FlxSprite = new FlxSprite(-600, 200).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.WHITE);
					bg.antialiasing = true;
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'Brim':
				dad.setGraphicSize(Std.int(dad.width - 300));
				dad.setGraphicSize(Std.int(dad.height - 300));
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
				dad.x -= 900;
				dad.y -= 150;		
		}
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - KE " + "CUSTOM BUILD" + " - " + (FlxG.save.data.etternaMode ? "E.Mode" : "FNF"), 16);
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						///a shitty way to add cpu strums
					{
						cpuStrums.forEach(function(spr:FlxSprite)
						{
